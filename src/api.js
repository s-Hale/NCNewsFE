export const fetchArticles = topic => {
  if (topic) {
    return fetch(`https://still-meadow-57011.herokuapp.com/api/topics/${topic}/articles`).then(
      
      res => res.json()
    );
  } else {
    return fetch(`https://still-meadow-57011.herokuapp.com/api/articles`).then(res => res.json());
  }
};

export const fetchArticleById = articleID => {
  return fetch(`https://still-meadow-57011.herokuapp.com/api/articles/${articleID}`).then(res => 
    res.json()
  )
}

export const fetchArticlesByUsername = username => {
  return fetch(`https://still-meadow-57011.herokuapp.com/api/articles/${username}`).then(res => 
    res.json()
  )
}

export const fetchUserInfo = userInfo => {
  return fetch(`https://still-meadow-57011.herokuapp.com/api/users/${userInfo}`).then(res => 
    res.json()
  );
};

export const fetchTopics = () => {
  return fetch("https://still-meadow-57011.herokuapp.com/api/topics").then(res => res.json());
};

export const fetchComments = articleID => {
  return fetch(`https://still-meadow-57011.herokuapp.com/api/articles/${articleID}/comments`).then(
    res => res.json()
  )
};
